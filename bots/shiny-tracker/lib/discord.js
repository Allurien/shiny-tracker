...
async function sendRestockAlert(send, products) {
  ...
  // Wrap the URL in <...> so Discord doesn't auto-embed it as a separate
  // unfurl card — the product embeds below should be the only embeds.
  const headline =
    `\u{1F514} **New Restock Alert!** (${products.length} new item${products.length > 1 ? "s" : ""})\n` +
    `\u{1F517} View on DAC: <${RESTOCK_PAGE_URL}>`;
